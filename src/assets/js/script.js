document.addEventListener("DOMContentLoaded", function () {
  if (!annyang) {
    return alert(
      "Lo siento, tu navegador no soporta el reconocimiento de voz :("
    );
  }
  ...
  let comandos = {
    ...
    hola: () => {
      loguearComandoReconocido(`Holass mundo!`);
    },
    ...
    "enviar correo a *usuario": (usuario) => {
      let usuarioCorregido = usuario
        .replace(/\ /g, "")
        .replace(/arroba/g, "@")
        .toLowerCase();
      loguearComandoReconocido(
        `Originalmente es ${usuario} pero creo que el correcto es ${usuarioCorregido}`
      );
    },
    "mi nombre es *nombre y tengo *anios años": (nombre, anios) => {
      loguearComandoReconocido(
        `Hola ${nombre} es genial que tu edad sea ${anios} :)`
      );
    },
  };
  ...
  annyang.addCallback("result", (frases) => {
    loguearVozDetectada(
      `<strong>Probablemente has dicho: </strong> <br> ${frases.join("<br>")}`
    );
  });
